<template>
  <div class="px-[16px] md:px-[42px]">
      ...
      <div class="flex items-center flex-wrap justify-center gap-8">
          <a v-for="(v, i) in dataTempat" :key="i" :href="`${cekKategoriSlug(v.kategori)}/${v.slug}`" class="w-full md:w-[30%] rounded-2xl overflow-hidden bg-white shadow-md cursor cursor-pointer-card" style="height: auto;">
          ...
          <div class="p-4">
              ...
              <a :href="`${cekKategoriSlug(v.kategori)}/${v.slug}`" class="ml-auto text-sm text-rose-500 hover:underline hover:decoration-from-font">SELENGKAPNYA!</a>
              </div>
          </div>
          </a>
      </div>
...
export default {
  ...
  cekKategoriImg(v){
    return v.includes('Ruang Rapat') ? 'kategori2.png' :  v.includes('Wedding') ? 'kategori4.png' : v.includes('Tenis') ? 'kategori1.png' :  'kategori3.png';
  },
  hitungJumlahTempat() {
